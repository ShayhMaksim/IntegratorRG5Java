...
public class TMatrix implements IMatrix {
	...
	public  static IMatrix E(int k) {   //////////åäèíè÷íàÿ ìàòðèöà
	...
	public IMatrix Add(IMatrix a) {
		// TODO Auto-generated method stub
		try {
			if (a.GetColCount()!=this.GetColCount()) throw new Exception("Íåñîâìåñòèìûå ðàçìåðíîñòè");
			}
			catch(Exception ex)
			{
	...
	public IVector Mult(IVector a) {
		// TODO Auto-generated method stub
		try {
			if (a.GetSize()!=this.GetRowCount()) throw new Exception("Íåñîâìåñòèìûå ðàçìåðíîñòè");
			}
			catch(Exception ex)
			{
	...
	public double Det() {
		...
		mt2=this.Clone();
		/*ìàòðèöû mt1 è mt2 íåîáõîäèìû,÷òîáû íå èñïîðòèòü ïîëó÷àåìóþ ìàòðèöó "this",êîòîðàÿ ìîæåò áûòü èñïîëüçîâàíà â äðóãèõ îáëàñòÿõ ïðîãðàììû
		ñòðîèì òðåóãîëüíóþ ìàòðèöó*/
		for(int i=0;i<this.GetColCount();i++)
		{
			...
			/*êîïèðîâàíèå ðàáî÷åé ìàòðèöû â ñòàòè÷åñêóþ ìò2,íåîáõîäèìî äëÿ êîððåêòíîãî ðàñ÷åòà êîýôôèöèåíòîâ óìíîæåíèÿ ñòðîê ïåðåä âû÷èòàíèåì*/
	...
	public IMatrix Invers() {
		if (this.GetRowCount()!=this.GetColCount()) {
			System.out.println("íå êâàäðàòíàÿ ìàòðèöà");
			return null;
		}
	...
	{	
		if (this.Silvestr()==false) {
			System.out.println("íå ïîëîæèòåëüíà îïðåäåëåíà");
			return null;
		}
